...
class NeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, time, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        #time = time.expand(xyzs.size()[0],1)
        ts = time.expand(xyzs.shape[:2])
       ...
       # print('time shape:',time.size())
        ...
        if upsample_steps > 0:
            with torch.no_grad():
                ...
                new_ts = time.expand(xyzs.size()[0],1)
        ...
        ts1 = time.expand(xyzs.shape[:2])
       # print('dirs shape:', dirs.size())
       # print('ts shape:', ts1.size())
        for k, v in density_outputs.items():
            density_outputs[k] = v.view(-1, v.shape[-1])

        rgbs = self.color(xyzs.reshape(-1, 3), dirs.reshape(-1, 3), ts1.reshape(-1, 1), mask=mask.reshape(-1), **density_outputs)
    ...
    def run_cuda(self, rays_o, rays_d, time, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
        ...
        if self.training:
            ...
            # print('xyzs cuda:',xyzs)

            #plot_pointcloud(xyzs.reshape(-1, 3).detach().cpu().numpy())
            #ts = time.expand(xyzs.shape[:2])
            ts = time.expand(xyzs.size()[0],1)
            ts = ts.to(device)
            ...
            rgbs = self.color(xyzs, dirs, ts.reshape(-1, 1), **density_outputs)
        ...
        if  self.tv_loss:
            tv_loss = self.total_variation_loss(time) * self.tv_loss_weight
        ...
        for xi, xs in enumerate(X):
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    time = torch.FloatTensor(torch.rand(1))
                    ...
                    for cas in range(self.cascade):
                        ...
                        # query density
                        sigmas = self.density(cas_xyzs.to(tmp_grid.device),ts)['sigma'].reshape(lx, ly, lz).detach()
    ...
    def total_variation_loss(self, time, min_resolution=16, max_resolution=2048, log2_hashmap_size=19, n_levels=16):
        ...
        for level in range(n_levels):
            ...
            # Get resolution
            b = math.exp((math.log(max_resolution)-math.log(min_resolution))/(n_levels-1))
            ...
            # Sample cuboid
            min_vertex = torch.randint(0, resolution-cube_size, (3,)) #torch.rand(1)
            idx = min_vertex + torch.stack([torch.arange(cube_size+1) for _ in range(3)], dim=-1) 
            idx = (idx -(resolution)//2)  /  ((resolution)//2)

            cube_indices = torch.stack(torch.meshgrid(idx[:,0], idx[:,1], idx[:,2]), dim=-1) #[N,N,N,3]
            # print('cube_indices:',cube_indsices,cube_indices.shape)

            xyzs = cube_indices.reshape(-1, 3) #[N*N*N,3]
            xyzs = xyzs.to(device='cuda')
            # print('xyzs:',xyzs)
            #sigma tv loss # time
            ts = time.expand(xyzs.size()[0],1) #[N*N*N,1]
            ts = ts.to(device='cuda')
            density_outputs = self.density(xyzs.reshape(-1, 3), ts.reshape(-1, 1))
            cube_embeddings = density_outputs['sigma']#[N*N*N, 1]
            cube_embeddings = cube_embeddings.reshape(cube_indices.size()[0],cube_indices.size()[1],cube_indices.size()[2],1)
            # print('cube_embeddings:',cube_embeddings)

            ###
            # # 
            # cube_embeddings = self.girdfeat(xyzs) #[N*N*N, 2*16]
            # # print('cube_embeddings:',cube_embeddings)

            # cube_embeddings = cube_embeddings.reshape(cube_indices.size()[0],cube_indices.size()[1],cube_indices.size()[2],32)
            # start = 2 * level
            # end = 2 * level + 2
            # cube_embeddings = cube_embeddings[:,:,:,start:end]
            ...
            loss_list.append((tv_x + tv_y + tv_z)/cube_size)
            # print('tv loss:',(tv_x + tv_y + tv_z)/cube_size)

        return sum(loss_list)


    def render(self, rays_o, rays_d, time, staged=False, max_ray_batch=4096, bg_color=None, perturb=False, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        ...
        if staged and not self.cuda_ray:
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    results_ = _run(rays_o[b:b+1, head:tail], rays_d[b:b+1, head:tail], time, bg_color=bg_color, perturb=perturb, **kwargs)
            ...
            results = _run(rays_o, rays_d, time, bg_color=bg_color, perturb=perturb, **kwargs)
