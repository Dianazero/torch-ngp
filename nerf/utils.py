...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, time, staged=False, bg_color=bg_color, perturb=True, **vars(self.opt))
    
        pred_rgb = outputs['image']
        
        tv_loss = outputs['tv_loss']
        ...
        return pred_rgb, gt_rgb, loss ,tv_loss
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        for epoch in range(self.epoch, max_epochs + 1):
            self.epoch = epoch
            # if self.epoch > (max_epochs + 1)*2/3:
            #     self.sample_mode = 'ist_map'
    ...
    def train_gui(self, train_loader, step=16):
        ...
        for _ in range(step):
            ...
            with torch.cuda.amp.autocast(enabled=self.fp16):
                preds, truths, loss , tv_loss = self.train_step(data)
            #add tv loss
            if  self.tv_loss:
                loss = loss + tv_loss
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            if self.local_rank == 0:
                ...
                if self.scheduler_update_every_step:
                    pbar.set_description(f"loss={loss_val:.4f} ({total_loss/self.local_step:.4f}), tv_loss={tv_loss:.6f} , lr={self.optimizer.param_groups[0]['lr']:.6f}")
                else:
                    pbar.set_description(f"loss={loss_val:.4f} ({total_loss/self.local_step:.4f}),tv_loss={tv_loss:.6f} , ")
                pbar.update(loader.batch_size)
