...
class NeRFNetwork(NeRFRenderer):
    def __init__(self,
        ...
        self.encoder_time, self.out_dim_time = get_encoder("frequency", input_dim = 1, multires = 8)
        # self.out_dim_time = 64
        # time_net = []
        # for l in range(self.num_layers_time):
        #     if l == 0:
        #         in_dim = 1  #self.in_dim_time 
        #     else:
        #         in_dim = 256 #hidden_dim
            
        #     if l == self.num_layers_time - 1:
        #         out_dim = self.out_dim_time #self.in_dim_time
        #     else:
        #         out_dim = 256 #hidden_dim
 ...
 #       t = self.encoder_time(t)
    ...
    def girdfeat(self, x):
        # x: [N, 3], in [-bound, bound]
        feat = self.encoder(x, bound=self.bound)
        return feat